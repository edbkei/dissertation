...
DEFAULT_EXTERNAL_HOST = "34.134.152.190" #here localhost or 34.134.152.190
DEFAULT_PYTHON_PATH = ".."
...
class DemoAgent:
    ...
    def _terminate(self):
        if self.proc and self.proc.poll() is None:
            self.proc.terminate()
            try:
                self.proc.wait(timeout=0.5)   # here, before 0.5
    ...
    async def detect_process(self, headers=None):
        async def fetch_status(url: str, timeout: float, headers=None):
            ...
            text = None
            start = default_timer()
            async with ClientSession(timeout=ClientTimeout(total=3.0)) as session: # here, before 3.0
