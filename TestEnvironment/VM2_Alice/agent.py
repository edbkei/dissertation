...
DEFAULT_EXTERNAL_HOST = "104.198.179.213" # here localhost
DEFAULT_PYTHON_PATH = ".."
PYTHON = os.getenv("PYTHON", sys.executable)

START_TIMEOUT = float(os.getenv("START_TIMEOUT", 90.0)) # here before 30.0s
...
if RUN_MODE == "docker":
    ...
    DEFAULT_PYTHON_PATH = "."
    print("RUN_MODE = docker:  DEFAULT_EXTERNAL_HOST", DEFAULT_EXTERNAL_HOST)
elif RUN_MODE == "pwd":
    # DEFAULT_INTERNAL_HOST =
    DEFAULT_EXTERNAL_HOST = os.getenv("DOCKERHOST") or "host.docker.internal"
    DEFAULT_PYTHON_PATH = "."
    print("RUN_MODE = pwd:  DEFAULT_EXTERNAL_HOST", DEFAULT_EXTERNAL_HOST)
...
class DemoAgent:
    def __init__(
        ...
        **params,
        ...
        self.external_host = DEFAULT_EXTERNAL_HOST # here: external_host or DEFAULT_EXTERNAL_HOST
        ...
        print("self.admin_url: ",self.admin_url)
        if AGENT_ENDPOINT:
            self.endpoint = AGENT_ENDPOINT
            print("self.endpoint of AGENT_ENDPOINT: ",self.endpoint) # here
        elif RUN_MODE == "pwd":
            self.endpoint = f"http://{self.external_host}".replace(
                "{PORT}", str(http_port)
            )
            print("self.endpoint of RUN_MODE=pwd: ",self.endpoint) # here
        else:
            self.endpoint = f"http://{self.external_host}:{http_port}" # here
            print("self.endpoint of elsewhere: ",self.endpoint) # here
    ...
    def get_agent_args(self):
        result = [
            ...
            ("--inbound-transport", "http", "0.0.0.0", str(self.http_port)), #here, before 0.0.0.0
            ("--outbound-transport", "http"),
            ("--admin", "0.0.0.0", str(self.admin_port)), # here, before 0.0.0.0
            "--admin-insecure-mode",
    ...
    def _terminate(self):
        if self.proc and self.proc.poll() is None:
            self.proc.terminate()
            try:
                self.proc.wait(timeout=1.0) # before 0.5s
