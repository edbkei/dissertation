...
async def main(
    ...
    try:
        ...
        with log_timer("Connect duration:"):
            ...
            await asyncio.wait_for(faber.detect_connection(), 120) #2022-03-29 here before 30
        ...
        async def check_received_creds(agent, issue_count, pb):
            ...
            while True:
                ...
                if reported == complete:
                    await asyncio.wait_for(agent.update_creds(), 120)   # here before 30
                    continue
        ...
        async def check_received_pings(agent, issue_count, pb):
            ...
            while True:
                ...
                if complete == reported:
                    await asyncio.wait_for(agent.update_pings(), 120) # here, before was 30
                    continue
                if iter_pb and complete > reported:
                    try:
                        while next(iter_pb) < complete:
        ...
        faber.log(f"Average time per {item_long}: time_duration/count (latency) {avg:.2f}s (tps: {1/avg:.2f}/s)")
