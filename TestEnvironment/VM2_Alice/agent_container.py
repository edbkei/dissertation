...
class AriesAgent(DemoAgent):
    ...
    async def handle_present_proof(self, message):  # Here
        state = message["state"]
        ...
        elif state == "presentation_received": # Here: Presentation Proof
            log_status("#27 Process the proof provided by X")
            log_status("#28 Check if proof is valid")
            log_status("#28a Should be here?")
            proof = await self.admin_POST(
                f"/present-proof/records/{presentation_exchange_id}/verify-presentation"
            )
            self.log("Proof =", proof["verified"])  # True False
    ...
    async def handle_present_proof_v2_0(self, message):
        ...
        if state == "request-received": # Holder role
            # prover role
            log_status(
                "#24 Query for credentials in the wallet that satisfy the proof request"
            )
        ...
        elif state == "presentation-received":  # Here: Presentation Proof at Issuer role, i.e. Faber Role
            # verifier role
            log_status("#27 Process the proof provided by X")
            log_status("#28 Check if proof is valid")
            log_status("#28x It is here")
...
async def test_main(
    ...
    try:
        ...
        # start the agents - faber gets a public DID and schema/cred def   # here check
