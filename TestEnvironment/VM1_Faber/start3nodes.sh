...
#
# *** WARNING: It is preferrable to run scripts below step by step. It is possible error if run in batch mode.
#              There is a bug in the script, channel number shall always be changing. The channel keeps alive even
#              removing docker instances. This should be an issue for Hyperledger community.
# n = channel number (e.g. 1..)
CHANNEL=8
#
