...
#ENV PYTHONPATH /home/ubuntu/go/src/github.com/edbkei/fabric-samples/asset-transfer-basic/application-javascript
#from token_manager import t_exists, t_create, t_read, t_update, t_delete, t_exists, t_all, t_transfer

#from ubuntu.go.src.github.com.edbkei.fabric-samples.asset-transfer-basic.application-javascript.token_manager import (
#   t_exists,
#   t_create,
#   t_read,
#   t_update,
    ...
    def generate_proof_request_web_request(
        ...
        if aip == 10:
            ...
            if cred_type == CRED_FORMAT_INDY:
                ...
                req_preds = [
                    # test zero-knowledge proofs -- Here
...
async def main(args):
    ...
    try:
        ...
        #await faber_agent.generate_invitation(display_qr=True, wait=True) #here
        ...
        options += "    (X) Exit?\n[1/2/3/4/7/{}{}T/X] ".format(
            "5/6/" if faber_agent.revocation else "",
            "W/" if faber_agent.multitenant else "",
        )
        #print("faber_agent.revocation= ",faber_agent.revocation) # here, revoke
        #print("options =", options)                              # here, revoke
        async for option in prompt_loop(options):
            #print("option inside async for option=", option) # here, revoke
            ...
            if option is None or option in "xX":
            ...
            elif option == "7":  # Energy Token Management
                ...
                if option=="1":
                   ...
                   toowner=input('Type to whom you want to transfer your asset (e.g. Tom_id)')
                   url=url+"/t,"+asset+","+toowner
                   resp = requests.get(url, headers=headers)
                   log_msg(resp.text)
                   ...
                   range_start = 10**(n-1)
                   range_end = (10**n)-1
                   aux=randint(range_start, range_end)
                   asset="a"+str(aux)
                   finalConsumer="none"
                   status="ONCHAIN"
                   docType="asset"
                   appraisedValue=""
                   owner=input('Type the owner of token (e.g. Alice_id): ')
                   energyKWH=input('Type how much energy produced (e.g. 100) in KWH: ')
                   appraisedValue=input('Type estimate value for this energy (e.g. 1000) in unit: ')
                   created=asset+","+finalConsumer+","+energyKWH+","+status+","+owner+","+appraisedValue+","+docType
                   url=url+"/cc,"+created
                   creating="created token: "+created
                   resp = requests.get(url, headers=headers)
