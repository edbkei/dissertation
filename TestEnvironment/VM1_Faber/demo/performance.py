...
async def main(
    ...
    try:
        ...
        with log_timer("Connect duration:"):
            ...
            await asyncio.wait_for(faber.detect_connection(), 420) # 2022-03-25 here before 30
        ...
        async def check_received_creds(agent, issue_count, pb):
            ...
            while True:
                ...
                if reported == complete:
                    await asyncio.wait_for(agent.update_creds(), 420) # here, before was 30
                    continue
        ...
        async def check_received_pings(agent, issue_count, pb):
            ...
            while True:
                ...
                if complete == reported:
                    await asyncio.wait_for(agent.update_pings(), 420) # here, before was 30
                    continue
                if iter_pb and complete > reported:
                    try:
                        while next(iter_pb) < complete:
        ...
        recv_timer.stop()
        tps = issue_count / recv_timer.duration
        ...
        faber.log(f"Latency: Average time per {item_long}: {avg:.2f}s ") # here
        faber.log(f"Threads: {threads}") # here
        faber.log(f"Transactions per second: {tps}") # here
