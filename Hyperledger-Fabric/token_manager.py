...
def t_create(asset,installid,energykwh,ts,owner,appraisalvalue):
    p = check_output(['node', apps,'create',asset,installid,energykwh,ts,owner,appraisalvalue])
    return True
...
def t_update(asset,installid,energykwh,ts,owner,appraisalvalue):
    p = check_output(['node', apps,'update',asset,installid,energykwh,ts,owner,appraisalvalue])
    my_json=p.decode('utf8').replace("'", '"') # decode  from bytes to string
    start = my_json.find("Result: ") + len("Result: ")
    end = start+15
    substring = my_json[start:end]
